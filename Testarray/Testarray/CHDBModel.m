...
+ (BOOL)isExistInTable
{
    ...
    [chDB.dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
...
+ (BOOL)createTable
{
    ...
    CHDBTool *chDB = [CHDBTool shareInstance];
    [chDB.dbQueue inTransaction:^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
- (BOOL)save
{
    ...
    CHDBTool *dwqDB = [CHDBTool shareInstance];
    __block BOOL res = NO;
    [dwqDB.dbQueue inDatabase:^(FMDatabase *db) {
...
{
    for (CHDBModel *model in array) {
        if (![model isKindOfClass:[CHDBModel class]]) {
            return NO;
        }
    }
...
+ (BOOL)updateObjects:(NSArray *)array
{
    ...
    CHDBTool *chDB = [CHDBTool shareInstance];
    // 如果要支持事务
    [chDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (CHDBModel *model in array) {
            NSString *tableName = NSStringFromClass(model.class);
...
+ (BOOL)clearTable
{
     CHDBTool *chDB = [CHDBTool shareInstance];    __block BOOL res = NO;
    [chDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = NSStringFromClass(self.class);
...
+ (NSArray *)findAll
{
    ...
    [chDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            CHDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
